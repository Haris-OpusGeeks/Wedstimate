...
import {deletePackage, getListOfPackages} from "../../../Redux/Reducers/packageSlice.js";
import usePackageSelector from "../../../Redux/Selectors/usePackageSelector.js";
...
function ViewPackages() {
    ...
    const {packageItem: {packages, isLoading}} = usePackageSelector();
    ...
    useEffect(() => {
        if (blogToDelete) {
            ...
            dispatch(deletePackage(requestData))
                .then(async ()=>{
    ...
    const onPackageEdit =  (packageEdit) => {
        return async () => {
            if (localStorage.getItem('singleBlog')) {
                await localStorage.removeItem('singlePackageEdit');
                await localStorage.setItem('singlePackageEdit', JSON.stringify(packageEdit));
                navigate(`/dashboard/packages/edit/${packageEdit.id}`);
            } else {
                await localStorage.setItem('singlePackageEdit', JSON.stringify(packageEdit));
                navigate(`/dashboard/packages/edit/${packageEdit.id}`);
            }
    ...
    const columns = [
        {
            name: 'Sr No',
            selector: (row, index) => index + 1,  // Dynamically set serial number
            width: "100px",
        },
        {
            name: 'Package Name',
        ...
        },
        {
            ...
            cell: (row) => (
                <>
                    <Link className="btn" onClick={onPackageEdit(row)} title='Edit'>
                    ...
                    <div className="row">
                        ...
                        <div className="col-lg-4">
                            <div className="d-flex">
                                <Link className='btn' to={'new'}>Add Package</Link>
                            </div>
                        </div>
                    </div>

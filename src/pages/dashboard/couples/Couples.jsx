...
import {deleteCoupleUser, getCouples} from "../../../Redux/Reducers/profileSlice.js";
import useProfileSelector from "../../../Redux/Selectors/useProfileSelector.js";
import {convertUTCToLocal, convertUtcToLocalTime} from "../../../Redux/Utils/helper.js";
...
export default function Couples() {
    ...
    const {couples: {coupleList, isLoading}} = useProfileSelector();
    ...
    const [sortedCouples, setSortedCouples] = useState([]);
    ...
    const emailNavigation = async (email) => {
    ...
    const columns = [
        ...
        {
            name: 'Wedding Date',
            selector: row => convertUTCToLocal(row.weddingDate),
            sortable: true, // Enable sorting
        },
        {
            name: 'ZIP Code',
            ...
            cell: (row) => (
                <>
                    ...
                    <button className={"btn"} onClick={()=> {
                        emailNavigation(row.email)
                    }}>
                        <i className="bi bi-envelope"></i>
                    </button>
                    <button className="btn" onClick={() => deleteCouple(row.id)}>
    ...
    }, [coupleList, sortOption, isLoading]);


    const sortCouples = () => {
        const sorted = [...coupleList].sort((a, b) => {
            if (sortOption === 'name') {
                return a.fullName.localeCompare(b.fullName); // Sort by name
            } else if (sortOption === 'dateCreated') {
                return new Date(a.createdOn) - new Date(b.createdOn); // Sort by date created
            }
            // Add more sorting options if needed
            return 0;
        });
        setSortedCouples(sorted);
    };
                    ...
                    <div className="row">
                        ...
                        <div className="col-lg-6">
                            <div className="d-flex">
                                ...
                                {zipCode && zipCode.length === 5 ? <div className={"pill"}><h6>{zipCode}</h6> <button onClick={returnToCouple()} className={"btn cancelBtn"}><i
                                    className="bi bi-x"></i></button></div> : null}
                                <Link to={"/dashboard/create-couples"} type="button" className='btn'>Add Couple</Link>
                                <div className="dropdown">
                                    ...
                                    <ul className="dropdown-menu">
                                        ...
                                        {/*<li><button className="dropdown-item" href="#">Sort By Alphabets</button></li>*/}
                                        {/*<li><button className="dropdown-item" href="#">Sort By members type</button></li>*/}
                                    </ul>
                                </div>
                            </div>
                        </div>
